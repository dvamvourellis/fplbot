...
from fplbot.knowledge_base.data_utils import (
    process_current_data,
    get_fixture_data,
    create_player_history_agg,
)
...
from fplbot.bot_agent.templates import MULTI_DF_PREFIX, MULTI_DF_PREFIX_WITH_HIST
...
class fplAgent:
    ...
    def _fetch_data(self):
        ...
        if self.load_history == True:
            if self.update_data == True:
                player_data_agg_dict_final = create_player_history_agg(
                    data_dict["id2player"],
                    data_dict["id2team"],
            ...
            self.players_season_history = player_data_agg_dict_final[
                "players_season_history"
            ]
    ...
    def _init_agent(self):
        ...
        if self.load_history == True:
            dfs.append(self.players_season_history)
            dfs.append(self.players_gw_history)
            prefix = MULTI_DF_PREFIX_WITH_HIST

        self.agent = create_pandas_multidf_with_memory(
            self.llm, dfs, verbose=self.verbose, prefix=prefix
        )
